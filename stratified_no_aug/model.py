...
#TANPA AUGMENTASI - untuk perbandingan dengan model yang menggunakan augmentasi
...
def create_datasets_with_batch_size(data_dir, batch_size, train_ratio=0.7, val_ratio=0.15, test_ratio=0.15):
    ...
    print(f"\n{'='*60}")
    print("STRATIFIED DATASET SPLIT - NO AUGMENTATION")
    print(f"{'='*60}")
    ...
    # TANPA AUGMENTASI - langsung batch dan prefetch
...
# Setup tuner
tuner_dir = "./stratified_no_aug/tuner_results"
...
tuner = kt.RandomSearch(
    ...
    directory=tuner_dir,
    project_name="snake_vit_tuning_no_aug",
    overwrite=False
)
...
# Train final model
checkpoint_dir = "./stratified_no_aug/checkpoints"
...
plt.savefig('stratified_no_aug/confusion_matrix.png')
plt.show()
...
def plot_training(history):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('stratified_no_aug/training_history.png')
    plt.show()

plot_training(history)

# Save results
os.makedirs("stratified_no_aug", exist_ok=True)
with open("stratified_no_aug/tuning_results.txt", "w") as f:
    f.write("="*60 + "\n")
    f.write("VISION TRANSFORMER (ViT) - SNAKE CLASSIFICATION RESULTS\n")
    f.write("TANPA AUGMENTASI\n")
    f.write("="*60 + "\n\n")
    ...
    f.write("TIDAK ADA AUGMENTASI - Model dilatih dengan data asli saja\n\n")
...
print(f"\n✅ Results saved to 'stratified_no_aug/tuning_results.txt'")
# Save model
model.save("stratified_no_aug/snake_vit_model.keras")

# Convert to TFLite
converter = tf.lite.TFLiteConverter.from_keras_model(model)
converter.optimizations = [tf.lite.Optimize.DEFAULT]
tflite_model = converter.convert()
with open("stratified_no_aug/snake_vit_model.tflite", "wb") as f:
