# pake env vit_snake -> diganti jadi mobileNet
...
# Ensure keras import compatibility if needed
...
LOCAL_MODEL_PATH = os.path.join(MOBILENET_DIR, 'mobilenet_backbone')
TUNER_DIR = os.path.join(MOBILENET_DIR, 'tuner_results')
CHECKPOINT_DIR = os.path.join(MOBILENET_DIR, 'checkpoints')
os.makedirs(MOBILENET_DIR, exist_ok=True)
os.makedirs(TUNER_DIR, exist_ok=True)
os.makedirs(CHECKPOINT_DIR, exist_ok=True)

print("Using MobileNetV3 (ImageNet weights) as backbone. Saving outputs to:", MOBILENET_DIR)
...
# Preprocess single image returning (224,224,3) float32 preprocessed for MobileNetV3
def preprocess_image(image_path, label):
    def _load(path):
        ...
        try:
            ...
            # use mobilenet_v3 preprocess_input
...
tuner = kt.RandomSearch(
    ...
    directory=TUNER_DIR,
    project_name="snake_mobilenet_tuning",
    overwrite=False
)
...
# Callbacks (save weights under mobileNet)
callbacks = [
...
for batch in test_ds.unbatch():
    # each batch element is (image, label)
    lbl = batch[1].numpy()
...
cm_path = os.path.join(MOBILENET_DIR, 'confusion_matrix.png')
plt.savefig(cm_path)
plt.show()
...
def plot_training(history):
    ...
    plt.legend()
    plt.tight_layout()
    hist_path = os.path.join(MOBILENET_DIR, 'training_history.png')
...
# Save model (Keras)
model_save_path = os.path.join(MOBILENET_DIR, "snake_mobilenet_model.keras")
model.save(model_save_path)

# Convert to TFLite
tflite_save_path = os.path.join(MOBILENET_DIR, "snake_mobilenet_model.tflite")
...
print(f"✅ Done! All outputs saved under: {MOBILENET_DIR}")