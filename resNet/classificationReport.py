...
WEIGHTS_PATH = '/Users/ardhyantry/Documents/GitHub/snake-classification/resNet/checkpoints/best_model.weights.h5'

# Class names
class_names = ['Venomous', 'Non-Venomous']

# Preprocessing function for ResNet50V2
...
# Rebuild ResNet50V2 model architecture
print("Rebuilding ResNet50V2 model architecture...")
...
# Dropout from best hyperparameters (0.2)
...
# Use the optimizer from best hyperparameters (adam with lr=2e-05)
model.compile(
...
# Save to text file
output_file = 'resNet/classification_report.txt'
with open(output_file, 'w') as f:
    f.write("="*60 + "\n")
    f.write("RESNET50V2 - SNAKE CLASSIFICATION TEST SET EVALUATION\n")
    f.write("="*60 + "\n\n")
    
    f.write(f"Dataset: {DATASET_PATH}\n")
    f.write(f"Model Weights: {WEIGHTS_PATH}\n")
    f.write(f"Model Architecture: ResNet50V2 (ImageNet pretrained)\n")
    f.write(f"Test Set Size: {len(y_test)} images\n")
    f.write(f"  - Venomous: {np.sum(y_test == 0)}\n")
    f.write(f"  - Non-Venomous: {np.sum(y_test == 1)}\n\n")
    
    f.write("Best Hyperparameters:\n")
    f.write("  - Learning Rate: 2e-05\n")
...
plt.title('ResNet50V2 - Confusion Matrix (Test Set)')
plt.tight_layout()
plt.savefig('resNet/test_confusion_matrix.png', dpi=300, bbox_inches='tight')
print(f"✅ Confusion matrix saved as 'resNet/test_confusion_matrix.png'")
plt.show()

print("\n" + "="*60)
print("✅ ResNet50V2 evaluation completed successfully!")
print("="*60)
