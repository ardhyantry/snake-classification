# vgg16_pipeline_with_classification_report.py
# Environment: vit_snake (but now using VGG16)
import os
import sys
import numpy as np
...
DATASET_PATH = '/Users/ardhyantry/Documents/GitHub/snake-classification/dataset ularBaru'  # adjust
VGG_DIR = './vgg16'
LOCAL_MODEL_DIR = os.path.join(VGG_DIR, 'vgg16_backbone')
TUNER_DIR = os.path.join(VGG_DIR, 'tuner_results')
CHECKPOINT_DIR = os.path.join(VGG_DIR, 'checkpoints')
OUT_DIR = VGG_DIR
...
print("Using VGG16 backbone. Outputs will be saved to:", VGG_DIR)
...
# Preprocess: returns (224,224,3) float32, preprocessed by vgg16.preprocess_input
def preprocess_image(image_path, label):
    def _load(path):
        ...
        try:
            ...
            arr = tf.keras.applications.vgg16.preprocess_input(arr)  # channel-last
            return arr
...
tuner = kt.RandomSearch(
    ...
    directory=TUNER_DIR,
    project_name="snake_vgg16_tuning",
    overwrite=False
)
...
model_save_path = os.path.join(OUT_DIR, "snake_vgg16_model.keras")
model.save(model_save_path)
print("Saved Keras model to:", model_save_path)

tflite_save_path = os.path.join(OUT_DIR, "snake_vgg16_model.tflite")
