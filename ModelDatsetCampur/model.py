...
DATASET_PATH = '/Users/ardhyantry/Documents/GitHub/snake-classification/datasetCampur'  
...
def build_model(hp):
    ...
    outputs = vit(inputs).pooler_output
...
# Setup tuner
tuner_dir = "./ModelDatsetCampur/tuner_results"
...
# Train final model
checkpoint_dir = "./ModelDatsetCampur/checkpoints"
...
plt.savefig('ModelDatsetCampur/confusion_matrix.png')
plt.show()
...
def plot_training(history):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('ModelDatsetCampur/training_history.png')
    plt.show()

plot_training(history)

# Save results
os.makedirs("ModelDatsetCampur", exist_ok=True)
with open("ModelDatsetCampur/tuning_results.txt", "w") as f:
    f.write("="*60 + "\n")
...
print(f"\n✅ Results saved to 'ModelDatsetCampur/tuning_results.txt'")
# Save model
model.save("ModelDatsetCampur/snake_vit_model.keras")

# Convert to TFLite
converter = tf.lite.TFLiteConverter.from_keras_model(model)
converter.optimizations = [tf.lite.Optimize.DEFAULT]
tflite_model = converter.convert()
with open("ModelDatsetCampur/snake_vit_model.tflite", "wb") as f:
